...
export class XThreadPostError extends Error {
  postedIds: string[];

  constructor(message: string, postedIds: string[]) {
    super(message);
    this.name = "XThreadPostError";
...
export async function postThreadToX(posts: string[]): Promise<string[] | null> {
  ...
  for (const [index, text] of posts.entries()) {
    try {
      ...
      throw new XThreadPostError(`X thread failed at post ${index + 1}: ${message}`, ids);
    }
  }
